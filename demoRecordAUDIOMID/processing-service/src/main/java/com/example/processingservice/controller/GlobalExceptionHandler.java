...
public class GlobalExceptionHandler {
    ...
    private Map<String, Object> resolveDetails(ErrorCode code, HttpServletRequest request) {
        if (request == null) {
            return null;
        }
        if (code != ErrorCode.ANALYSIS_NOT_READY && code != ErrorCode.TRANSCRIPT_NOT_READY) {
            return null;
        }
        Matcher matcher = MEETING_RESOURCE_PATH.matcher(request.getRequestURI());
        if (!matcher.matches()) {
            return null;
        }
        return Map.of("meetingId", matcher.group(1));
    }
