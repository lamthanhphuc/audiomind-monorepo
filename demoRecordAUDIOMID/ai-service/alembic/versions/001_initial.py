...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_transcripts_meeting_id"), "transcripts", ["meeting_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "technical_terms", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
        sa.Column(
    ...
    op.create_index(
        op.f("ix_analysis_meeting_id"), "analysis", ["meeting_id"], unique=True
    )
