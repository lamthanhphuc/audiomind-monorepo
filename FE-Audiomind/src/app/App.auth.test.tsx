...
const audioRecorderMock = {
  ...
  stopRecordingGraceful: vi.fn().mockResolvedValue({
    ...
    mimeType: 'audio/webm',
    extension: 'webm',
    recorded: { blob: new Blob(), mimeType: 'audio/webm', extension: 'webm' },
  }),
...
describe('App auth entry', () => {
  ...
  it('redirects back to login after successful register without an access token', async () => {
    ...
    expect(container.textContent).toContain('Đăng ký thành công. Bạn được dùng gói Standard miễn phí 3 ngày sau khi đăng nhập.')
  })
